...
void create_project(const std::filesystem::path& target_dir, const std::string& name) {
    ...
    source << "}\n\n";
    source << "func code(state: State) -> int {\n";
    source << "    match state {\n";
    source << "        State::Idle => {\n";
    source << "            return 0;\n";
    source << "        }\n";
    ...
    source << "    match job.state {\n";
    source << "        State::Idle => {\n";
    source << "            io::println(\"idle\");\n";
    source << "        }\n";
    source << "        State::Ready => {\n";
    ...
    source << "        }\n";
    source << "    }\n";
    source << "    match states[2] {\n";
    source << "        State::Idle => {\n";
    source << "            io::println(\"zero\");\n";
    source << "        }\n";
    source << "        State::Ready => {\n";
    source << "            io::println(\"one\");\n";
    source << "        }\n";
    source << "        State::Done => {\n";
    source << "            io::println(\"two\");\n";
    source << "        }\n";
    source << "    }\n";
    source << "    io::println(code(State::Done));\n";
    source << "}\n";
}
