...
void create_project(const std::filesystem::path& target_dir, const std::string& name) {
    ...
    source << "    label: string,\n";
    source << "}\n\n";
    source << "func finish(code: int) -> Result {\n";
    source << "    if code == 0 {\n";
    source << "        return Result::Ok(7);\n";
    source << "    }\n";
    source << "    return Result::Err(\"bad\");\n";
    source << "}\n\n";
    source << "func describe(result: Result) -> string {\n";
    source << "    match result {\n";
    source << "        Result::Ok => {\n";
    source << "            return \"ok\";\n";
    source << "        }\n";
